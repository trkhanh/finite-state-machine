import { boolean, option } from "yargs";
import StateMachine from "../../StateMachine";

export default function Config(options = {}) {
    "scope start initial final invalid errors".match(/\w+/g).forEach((name) => {
    ...
    // order
    this.order = option.order || StateMachine.getDefaultOrder();

    // default
    this.defaults = Object.assign(
        {
            // allow user to specify alternate triggers for event an actions ids
            action: "start",
            state: "enter",
        },
        options.defaults
